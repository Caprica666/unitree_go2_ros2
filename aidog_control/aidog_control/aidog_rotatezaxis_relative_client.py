...
class RotateZAxisRelativeClient(Node):
    ...
    def send_goal(self, order):
        ...
        goal_msg.turn_angle = 30.0 / 360 # Convert degrees to radians
        goal_msg.start_angle = 0.0
        goal_msg.angular_velocity = 0.1  # radians per second
        self.get_logger().info('Sending goal with turn_angle: {0}, start_angle: {1}, angular_velocity: {2}'.format(
            goal_msg.turn_angle, goal_msg.start_angle, goal_msg.angular_velocity))
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info('Result: {0}'.format(result.end_angle))
        rclpy.shutdown()

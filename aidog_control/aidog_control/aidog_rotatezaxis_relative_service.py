...
class RotateZAxisRelativeService(Node):
    ...
    def handle_rotatezaxis(self, request, response):
        angular_velocity = request.angular_velocity
        turn_angle = request.turn_angle
        response.atend = False
        if turn_angle > 0:
            max_angle = request.end_angle - request.start_angle
            if max_angle < 0:
        ...
        response.last_angle = request.start_angle + turn_angle
        response.elapsed_time = duration
        response.message = 'Rotation successful: last_angle {0} elapsed_time {1}'.format(response.last_angle, duration)
